...
const buySellItem = async (req, res) => {
	...
	const newCoins = Math.max(char.coins + item.coinCost * costFactor, 0);

	if (isBuying) {
		char.inventory.push(item);
	} else {
		if (!char.inventory.find((i) => i._id === item.id)) {
...
const equipItem = async (req, res) => {
	...
	if (!item) {
		res.status(400).json({
			message: "No valid item present in request body",
		});
		return;
	}
	...
	let oldGear = {};
	let newGear = inv.find(
		(equipped) => equipped.type == gearType && equipped.name == item.name,
	);
	...
	switch (gearType) {
		case "weapon":
			if (gear.weapon) {
				...
				res.status(404).json({
					message: "Nothing found inside weapon slot",
				});
				return;
			}
			break;
		case "armor":
			if (gear.armor) {
				...
				res.status(404).json({
					message: "Nothing found inside armor slot",
				});
				return;
			}
			break;
		case "helmet":
			if (gear.helmet) {
				...
				res.status(404).json({
					message: "Nothing found inside helmet slot",
				});
				return;
			}
			break;
		case "shield":
			if (gear.shield) {
				...
				res.status(404).json({
					message: "Nothing found inside shield slot",
				});
				return;
			}
			break;
