    ...
    <div className={`${styles.marketItems} ${mode === "buy" ? styles.buyItems : styles.sellItems}`}>
      {items.map((item) => (
        <div 
          ...
          {mode === "buy" ? (
            <button className={styles.buyButton} onClick={() => {console.log("BUY ITEM:", item); onBuy(item)}}
            > Buy </button>
            ) : (
            <button className={styles.sellButton} onClick={() => {console.log("SELL ITEM:", item); onSell(item)}}
            > Sell </button>
          )}
        </div>
      ))}     
